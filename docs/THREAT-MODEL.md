...
| T-006 | Medium | A threat actor (TA2) with access to CloudWatch Logs or container stdout attempts to read sensitive military report content from application logs, leading to exposure of classified or sensitive report data. | Information Disclosure | Report data in logs | M-010 | Mitigate | Implemented. Bot logs message previews (80-100 chars) and metadata only, not full content or extracted fields. Deployers should scope CloudWatch Logs access via IAM. |
